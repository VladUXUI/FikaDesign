...
import { PageHero, Placeholder } from "./PageShell";
...
function ContactForm() {
  const [state, formAction, pending] = useActionState(sendContact, { ok: false, error: null });
  ...
  return (
    <form action={formAction} style={{
      ...
      {state.error && (
        <p role="alert" style={{ fontSize: 14, color: "var(--fika-danger)", margin: 0, lineHeight: 1.5 }}>
          {state.error}
        </p>
      )}
      <div style={{
        ...
        <p style={{ fontSize: 12, color: "var(--fg-3)", margin: 0, lineHeight: 1.5, maxWidth: 280 }}>
          By submitting, you agree to our <a href="/privacy" style={{ color: "var(--fika-orange-deep)" }}>Privacy Policy</a>. We only use your details to respond to your enquiry.
        </p>
